...
# Logic to split data into training and testing set
```{r}
...
# Summary statistics with interaction term between grid and era with and without 12 cylinders era

The model performs significantly better With R-squared values going from 0.27 to 0.32
Taking out the data from the 12 cylinders era further improves the R-squared values to 0.39
...
summary(simpleModel)
```
# Starting vs finishing plots for each era

Seems to show a weak but significant correlation between better starting positions with better finishing positions

All plots appear to have comparable distributions
...
V8KERSplot = F1DataFrame %>% filter(F1DataFrame$era == "90 degrees V8 + KERS") %>% ggplot(aes(x = grid, y = positionOrder, colour = era)) + 
  geom_point(position = "jitter", size = .8) +
  labs(title="starting vs finishing position by era", 
     y="finishing position", 
     x="starting position")
...
```{r}
NBModel <- Train
NBModel$Attrition <- if_else(NBModel$Attrition == "No", 0, 1)
boruta_output <- Boruta(Attrition ~ ., data=NBModel, doTrace=2)
